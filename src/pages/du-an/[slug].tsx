import { getAllNews, getCommon, getCooperation, getLinks, getProjectDetail, getProjectIds, getProjects, getServices } from '@/api/services'
...
function ProjectDetail({ slug }: InferGetStaticPropsType<GetStaticProps>) {
  ...
  const detail = useQuery({ queryKey: ['project-detail'], queryFn: () => getProjectDetail(slug) }).data
    ...
    <Layout links={links} cooperation={cooperation} services={services} projects={projects} common={commons[0]}>
      ...
      <div className='padding'>
        <Breadcrumbs className='w-full relative before:w-full before:h-0.5 before:absolute before:left-0 before:bg-gray-500 before:-bottom-2'>
          ...
          <Link className='text-blue-500' href={"/du-an"}>Dự án</Link>
...
export async function getStaticPaths() {
  const slugs = await getProjectIds()
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  await queryClient.prefetchQuery(['project-detail'], () => getProjectDetail(slug))
