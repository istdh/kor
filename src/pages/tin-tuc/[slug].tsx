import { getBlogDetail, getCommon, getCooperation, getLinks, getOtherBlog, getProjects, getServices, getSlugBlogs } from '@/api/services'
...
function NewsDetail({ slug }: InferGetStaticPropsType<GetStaticProps>) {
  ...
  const detail = useQuery({ queryKey: ['blog-detail'], queryFn: () => getBlogDetail(slug) }).data
  const otherBlogs = useQuery({ queryKey: ['other-blogs'], queryFn: () => getOtherBlog(slug) }).data
...
export async function getStaticPaths() {
  const slugs = await getSlugBlogs()
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const slug = `tin-tuc/${params?.slug! as string}`
  ...
  await queryClient.prefetchQuery(['blog-detail'], () => getBlogDetail(slug))
  await queryClient.prefetchQuery(['other-blogs'], () => getOtherBlog(slug))
