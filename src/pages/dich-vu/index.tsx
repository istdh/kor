...
function ServicesPage() {
  ...
  return (
    <Layout links={links} services={services} cooperation={cooperation} projects={projects} common={commons[0]}>
      <Head>
        <title>Dịch vụ -   {newName[0]} {newName[1]}</title>
      </Head>
          ...
          <div>
            <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4'>
              {services.map((item: any) => (
                <ServiceCard key={item.id} type='dich-vu' image_url={item.thumbnail.url} title={item.title} sub_description={item.description} slug={item.slug} />
              ))}
            </div>
            <ContactInformation />
          </div>
