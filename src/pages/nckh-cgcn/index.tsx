...
function Research() {
  ...
  const reSearchs = useQuery({ queryKey: ['researchs'], queryFn: getResearchs }).data
  ...
  return (
    <Layout links={links} cooperation={cooperation} services={services} projects={projects} common={commons[0]}>
      <Head>
        <title>Nghiên cứu khoa học - Chuyển giao công nghệ | {newName[0]} {newName[1]}</title>
      </Head>
      ...
      <div className="padding">
        <Breadcrumbs className='w-full relative before:w-full before:h-0.5 mb-5 before:absolute before:left-0 before:bg-gray-500 before:-bottom-2'>
          ...
          <p className='font-semibold'>NCKH-CGCN</p>
        </Breadcrumbs>
            ...
            <div>
              ...
              <ul className='flex flex-col gap-1 list-disc list-inside'>
                {services.slice(0, 5).map((item: any) => (
                  <li key={item.id}>
                    <Link className='text-blue-700' href={`/dich-vu/${item.slug}`}>
