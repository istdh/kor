import { getCommon, getCooperation, getLinks, getOtherResearch, getProjects, getResearchDetail, getResearchSlugs, getResearchs, getServices } from '@/api/services'
...
function NewsDetail({ slug }: InferGetStaticPropsType<GetStaticProps>) {
  ...
  const detail = useQuery({ queryKey: ['research-detail'], queryFn: () => getResearchDetail(slug) }).data
  const otherBlogs = useQuery({ queryKey: ['other-research'], queryFn: () => getOtherResearch(slug) }).data
    ...
    <Layout links={links} cooperation={cooperation} services={services} projects={projects} common={commons[0]}>
      ...
      <div className='padding'>
        <Breadcrumbs className='w-full relative before:w-full before:h-0.5 before:absolute before:left-0 before:bg-gray-500 before:-bottom-2'>
          ...
          <Link className='text-blue-500' href={"/nckh-cgcn"}>NCKH-CGCN</Link>
          ...
          <div className='mb-4'>
            ...
            <div className=' lg:w-[350px]'>
              <ul className='text-blue-700 flex-col flex gap-2 items-start list-disc list-inside text-lg'>
                {otherBlogs.slice(0, 5).map((item: any) => (
                  <li key={item.id} >
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const slug = `nckh-cgcn/${params?.slug! as string}`
  ...
  await queryClient.prefetchQuery(['reseachs'], getResearchs)
  await queryClient.prefetchQuery(['research-detail'], () => getResearchDetail(slug))
  await queryClient.prefetchQuery(['other-research'], () => getOtherResearch(slug))
